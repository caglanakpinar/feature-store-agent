"""The feature-engineering agent's web-search cache: a `web_search_cache` collection living
...
`data_engineer.dbs.web_search_cache` — this module only binds it to this agent's own
storage and embedder, so the two agents' cached searches never mix.

    from feature_engineering.dbs.web_search_cache import handle_prompt

    results, from_cache = await handle_prompt("recommended imputation for MNAR data")
"""
...
from feature_engineering.dbs.dataset import CHROMA_DIR, FAISS_DIR, SyntheticEmbedder
