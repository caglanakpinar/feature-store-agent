...
Building one is the two-line pattern `agent_builder`'s own docstring shows —
    ...
    agent = build_agent("data_reader_agent", configs)

— except every caller of the classes in `decider.py`, `workers.py` and `judges.py` writes
`DataReader.run(question=...)` rather than resolving the name and the config directory itself and
risking getting either wrong. `build_agent` decides which `BaseAgent` subclass actually runs a name
(`WorkerAgent`, `JudgerAgent`, `PlannerAgent`, ...) from that entry's configured `type`; nothing here
second-guesses that — a class in this package is a name and a config directory, not a type.

    from data_engineer.agents.workers import DataReader

    result = DataReader.run(question="what does this dataset need?", context=profile_summary)
...
the toolbox, the prompt directory — is expensive enough that rebuilding it on every `.run()` call would
cost real time for no benefit. Pass `configs=` or any override to `.build()`/`.run()` to force a rebuild
...
# `data_engineer/tools/dataset.py`'s own `CONFIG_DIR` resolves `db:` names against. An agent's prompt
# and tool paths are resolved relative to it the same way `build_agent` resolves everything else.
